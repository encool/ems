...
namespace esphome
{
  ...
  public:
    ...
    void setup() override
    {
      ESP_LOGI("filament_state_select", "setup");
      this->traits.set_options({"need_pull_back",
                                "need_send_out",
                                "on_use",
                                "idle"});
    ...
    void control(const std::string &value) override
    {
      ...
      if (value == "need_pull_back")
        new_state_enum = need_pull_back;
      else if (value == "need_send_out")
        new_state_enum = need_send_out;
      else if (value == "on_use")
        new_state_enum = on_use;
      else if (value == "idle")
        new_state_enum = idle;
      else
      {
    ...
    void publish_state_from_parent(_filament_motion_state_set current_app_state)
    {
      ...
      {
      case need_pull_back:
        state_str = "need_pull_back";
        break;
      case need_send_out:
        state_str = "need_send_out";
        break;
      case on_use:
        state_str = "on_use";
        break;
      case idle:
        ...
        break;
      default:
        ESP_LOGW("filament_state_select", "Unknown internal state: %d", (int)current_app_state);
        return;
      }
      if (this->state != state_str)
      {
        ESP_LOGI("filament_motor_state", "motor state %s to new moto state %s ", this->state.c_str(), state_str.c_str());
